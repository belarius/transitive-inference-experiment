...
var listCount = 1;
var listType = 0;
var trainTrialCount = 120;
var testTrialCount = 420;
var counterbalance = 1;
var trainBlockCount = 10;
var testBlockCount = 10;
...
function sessionStarter(){
  ...
  listCount = Number(document.getElementById("list_count").value);
  listType = Number(document.getElementById("list_type").value);
  trainTrialCount = Number(document.getElementById("train_trial_num").value);
  testTrialCount = Number(document.getElementById("test_trial_num").value);
  counterbalance = Number(document.getElementById("counterbal").checked);
  ...
  if(counterbalance == 1){
    trainBlockCount = trainTrialCount/((listLength-1)*2);
    testBlockCount = testTrialCount/((listLength-1)*(listLength));
  } else {
    trainBlockCount = trainTrialCount/(listLength-1);
    testBlockCount = testTrialCount/((listLength-1)*(listLength)/2);
  }
  ...
  // ====Create Array Of Stimulus Pairs====
  combination_array = Array();
  // ====Start Big Session Loop====
  for(lp=0;lp<listCount;lp++){
    // ====Server Query===
    listRequest = zeroPad(Number(listType),4) + "-" + zeroPad(Number(listLength),2) + "-" + zeroPad(Number(listID+lp),4);
    console.log(listRequest);
    dataHttp.open("POST", listRequest + ".dat", false);
    dataHttp.send();
    dataList = dataHttp.responseText.split(",");
    console.log("DatList Length: " + dataList.length);
   // Assign subject ID
    subject_id = dataList[0];
    // Assign pictures to list
    for(i=0;i<listLength;i++){
      pth = "pics/" + (i+(lp*listLength)) + ".jpg";
      A = {filepath: pth, id: dataList[i+(lp*listLength)+2], rank: i, correct: false};
      console.log(dataList[i+(lp*listLength)+2])
      picture_array.push(A);
    }
    // ====Trial Structure Setup====
    if(counterbalance == 1){
      // ====Training Trials====
      for(b=0;b<trainBlockCount;b++){
        temp_array = Array();
        for(i=0;i<(listLength-1);i++){
          session_length = session_length + 1;
          trial_number.push(session_length.toString());
          temp_array.push([picture_array[i+(lp*listLength)], picture_array[i+1+(lp*listLength)]]);
          session_length = session_length + 1;
          trial_number.push(session_length.toString());
          temp_array.push([picture_array[i+1+(lp*listLength)], picture_array[i+(lp*listLength)]]);
        }
        shuffle(temp_array);
        ...
        for(i=0;i<listLength;i++){
          for(j=0;j<listLength;j++){
            if(i != j){
              session_length = session_length + 1;
              trial_number.push(session_length.toString());
              temp_array.push([picture_array[i+(lp*listLength)], picture_array[j+(lp*listLength)]]);
            }
          }
        }
        shuffle(temp_array);
          ...
          for(j=i;j<listLength;j++){
            if(i != j){
              ...
              if(Math.random() < 0.5){
                ...
                temp_array.push([picture_array[j+(lp*listLength)], picture_array[i+(lp*listLength)]]);
              }
            }
          }
        }
