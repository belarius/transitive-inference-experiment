// Derived Lists Testing Task (Order Scrambled)
// client-side
...
function sessionStarter(){
  ...
  for(g=0;g<5;g++){
    for(b=0;b<4;b++){
      ...
      for(i=0;i<4;i++){
        ...
        temp_array.push([picture_array[i*5 + g], picture_array[(i+1)*5 + g]]);
        session_length = session_length + 1;
        trial_number.push(session_length.toString());
        temp_array.push([picture_array[(i+1)*5 + g], picture_array[i*5 + g]]);
      }
      shuffle(temp_array);
  ...
  for(g=0;g<5;g++){
    for(b=0;b<4;b++){
      ...
      for(i=0;i<4;i++){
        session_length = session_length + 1;
        trial_number.push(session_length.toString());
        temp_array.push([picture_array[i*5 + 4 - g], picture_array[(i+1)*5 + 4 - g]]);
        session_length = session_length + 1;
        trial_number.push(session_length.toString());
        temp_array.push([picture_array[(i+1)*5 + 4 - g], picture_array[i*5 + 4 - g]]);
      }
      shuffle(temp_array);
      for(i=0;i<temp_array.length;i++){
        combination_array.push(temp_array[i]);
  ...
  derived_type_list = [1,2,3,4,5];
  shuffle(derived_type_list);
 
  for(lst=0;lst<5;lst++){
    if(derived_type_list[lst]==1){
      list_order = [1,9,13,15,22];
      new_ranks = [0,5,10,15,20]; // ABCDE, TD=0, CR=5
    } else if (derived_type_list[lst]==2) {
      list_order = [3,7,10,16,24];
      new_ranks = [5,10,15,0,20]; // BCDAE, TD=3, CR=1
    } else if (derived_type_list[lst]==3) {
      list_order = [0,6,14,17,23];
      new_ranks = [0,20,10,15,5]; // AECDB, TD=5, CR=3
    } else if (derived_type_list[lst]==4) {
      list_order = [4,5,12,18,21];
      new_ranks = [15,20,0,10,5]; // DEACB, TD=7, CR=0
    } else if (derived_type_list[lst]==5) {
      list_order = [2,8,11,19,20];
      new_ranks = [20,15,10,5,0]; // EDCBA, TD=10, CR=0
    }
    for(b=0;b<4;b++){
      temp_array = Array();
      for(i=0;i<5;i++){
        for(j=0;j<5;j++){
          if(i != j){
            session_length = session_length + 1;
            trial_number.push(session_length.toString());
		    var B = {filepath: picture_array[list_order[i]].filepath, id: picture_array[list_order[i]].id, rank: new_ranks[i], correct: false};
		    var C = {filepath: picture_array[list_order[j]].filepath, id: picture_array[list_order[j]].id, rank: new_ranks[j], correct: false};
			var temp_case = [B,C];
            temp_array.push(temp_case);
          }
        }
      }
      shuffle(temp_array);
